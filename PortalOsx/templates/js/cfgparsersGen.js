...
function MontaListaConfiguracoes()
{
	...
	//else alert ("Servidor n?o enviou a lista de Configura??es!");
}
...
function MontaListaServidores()
{
	...
	if (ListaServidores.charAt(0) != "$")	
	{
		...
		{
//			alert (aListaServidores[i]);
			document.form1.ListaServidores[i] = new Option(aListaServidores[i], "", false, false);
		}
		document.form1.ListaServidores.selectedIndex = -1;
	}
	else alert ("Servidor nãoo enviou a lista de Servidores de Processos!");
}

function MontaListaParsersGen()
{
	ListaParsers = document.form1.ListaParsersNomes.value;
	if (ListaParsers.charAt(0) != "$")	
	{
		aListaParsers = ListaParsers.split(";");
		
		while (document.form1.ListaParsers.length > 0)
			document.form1.ListaParsers.options[0] = null;

		for (i = 0; i < aListaParsers.length; i++)
		{
			document.form1.ListaParsers[i] = new Option(aListaParsers[i], "", false, false);
		}
		document.form1.ListaParsers.selectedIndex = -1;
	}
	else alert ("Servidor não enviou a lista de Parsers!");
}
...
/*
		// Verifica??o do vetor de relacionametos (aListaRelac)
...
*/
	...
	//else alert ("Servidor n?o enviou a lista de Relacionamentos!");
}

/*
 *	Seleciona a configura??o correspondente ao nome da configura??o selecionada em ListaConfiguracoes
 */
function SelecionaConfiguracao()
{
	...
	if (Indice != -1)
	{
		...
		for (i = 0; i < document.form1.ListaParsers.length; i++)
		{
			//alert (aListaRelac[Indice][2] +" - "+ document.form1.ListaParsers.options[i].text);
			if (aListaRelac[Indice][2] == document.form1.ListaParsers.options[i].text)
			{
				document.form1.ListaParsers.selectedIndex = i;
				//alert (document.form1.ListaParsers.options[i].text);
				break;
			}
		}
...
function VerificaCampo(NomeDoCampo, ValorDoCampo, TamMinimo, TamMaximo)
{
   ...
   if (ValorDoCampo.substring(0,3) != "pg_")
      return NomeDoCampo + " deve iniciar com \'pg_\'!";
...
function Inclui(Operacao)
{
   ...
   else
   {
	...
	{	
		...
		// Verifica se tem conversor
		Indice = document.form1.ListaParsers.selectedIndex;
		if (Indice == -1)
		{
			alert ("É necessário associar a nova configuração a um conversor!");
			return false;
		}
		...
		if (Bilhetadores == "")
		{
			...
			Bilhetadores = Bilhetadores.substring(0, Bilhetadores.length-1); // Retira o ?ltimo ";"

		// Preenche o formul?rio 2
		if (Operacao == "inclusao")
		{
			...
			document.form2.parser.value = document.form1.ListaParsers.options[document.form1.ListaParsers.selectedIndex].text;
		...
		else
		{
			...
			document.form4.parser.value = document.form1.ListaParsers.options[document.form1.ListaParsers.selectedIndex].text;
...
function Exclui()
{
	...
	if (Indice != -1)
	{
		// Preenche o formul?rio 3
		document.form3.configuracao.value = document.form1.ListaConfiguracoes.options[Indice].text;
		document.form3.servidor.value = document.form1.ListaServidores.options[document.form1.ListaServidores.selectedIndex].text;
		return true;
	}
	else 
	{
