...
/**
 */
public class OpApresentaPermissoesRel extends OperacaoAbs 
{
     ...
     */
    public OpApresentaPermissoesRel()
    {
    }
    ...
    public boolean iniciaOperacao(String p_Mensagem)
    {
        ...
        try
        {
            setOperacao("Apresenta Permiss&oatilde;es");
            ...
            else
            {
                ...
                m_Args[2] = "src=\"/PortalOsx/templates/js/permissoesrel.js\"";
                m_Args[3] = "onLoad=\"MontaListaPerfis()\"";
                m_Args[4] = "permissaodeacesso.gif";
                m_Args[5] = m_Html.criaFormulario(NoUtil.getNo().getDiretorioDefs().getS_DIR_TMPL_FORMS(), "permissoesrel.form", montaFormulario(p_Mensagem));
        ...
        {
            System.out.println("OpApresentaPermissoesRel - iniciaOperacao(): " + Exc);
            Exc.printStackTrace();
            return false;
        }
    }
...
//                                       + "Arquivo PermissoesRel.txt pode estar corrompido.\n" + "------------------------------------------------\n");
        ...
        for (i = 0; i < QtdElem; i++)
        {
            ...
            for (j = 0; j < ListaTipoRel.size(); j++)
            {
                Colunas = new Vector(3);
                if (j == 0) Colunas.add("<b>" + Perfil.getPerfil() + "</b>");
                else Colunas.add("");

                StringTokenizer st = new StringTokenizer((String) ListaTipoRel.elementAt(j), ":");
                TipoRelId = (String) st.nextElement();
                TipoRel = (String) st.nextElement();
    ...
    {
        String Header[] = { "Perfil", "Tipo de Relat&oacute;rio", "Perfis Acess&iacute;veis" };
        String Largura[] = { "80", "200", "265" };
        String AlinhamentoHeader[] = { "left", "left", "center" };
        String Alinhamento[] = { "left", "left", "left" };
        short Filtros[] = { 1, 1, 0 };
        Vector Linhas = null;

        Linhas = montaLinhas();
        m_Html.setTabela((short) 3, false);
    ...
    private void inicializa()
    {
    	 ...
    	 for (Iterator iter = NoUtil.listaDeNos.iterator(); iter.hasNext();)
         {
             try
             {
                ...
                PermissoesAux.addAll(noTmp.getConexaoServUtil().getPermissoesRel());
	            ListaTipoRel.addAll(noTmp.getConexaoServUtil().getListaTipoRelatorios());
             }
