...
/**
 */
public class OpCfgParsers extends OperacaoAbs 
{
   ...
   public boolean iniciaOperacao(String p_Mensagem) 
   {
      ...
      try
      {
         setOperacao("Configuração de Parsers");

         iniciaArgs(7);
         m_Args[0] = NoUtil.getNo().getDiretorioDefs().getS_DIR_TMPL_HTML();
         m_Args[1] = "formgen.htm";
         m_Args[2] = "src=\"/PortalOsx/templates/js/cfgparsers.js\"";
         m_Args[3] = "onLoad=\"Processa(0)\"";
         m_Args[4] = "cfgparsers.gif";
         m_Args[5] = m_Html.criaFormulario(NoUtil.getNo().getDiretorioDefs().getS_DIR_TMPL_FORMS(), "cfgparsers.form", montaFormulario(p_Mensagem));
      ...
      {
         System.out.println("OpCfgParsers - iniciaOperacao(): "+Exc);
         Exc.printStackTrace();
         return false;
      }
   }
   ...
   public String[] montaFormulario(String p_Mensagem) 
   {
      ...
      for (Iterator iter = NoUtil.listaDeNos.iterator(); iter.hasNext();)
      {
          try
          {
	          ...
	          {
	          	  Vector configuracoes = noTmp.getConexaoServUtil().getListaCfgParsers();
	          	  Vector parsers = noTmp.getConexaoServUtil().getListaParsers();
      ...
      if (Parsers != null)
      {
         ...
         for (short i = 0; i < QtdElem; i++)
         {
            Conversor = (ConversorCfgDef) Parsers.elementAt(i);
            if (i == QtdElem - 1)
