...
var ordem = "";
function init(){
	RelatorioAjax.isConsolidadoUso(retIsConsolidadoUso);
	RelatorioAjax.getNumPaginasRelUso(preenchePg);	
}
function retIsConsolidadoUso(valor){
	if(valor){
		$('titulo').innerHTML = "Estat&iacute;stica de Uso";
	}else{
		$('titulo').innerHTML = "Relat&oacute;rio de Uso";
	}
}
...
function preenchePg(numeroPaginas)
{
	...
	RelatorioAjax.executarRelatorioUso(mostrarRel,numPag);//chama o relatorio com a pagina inicial
}
...
function Pagina(pg)
{
	...
	if(msgErro == "")
	{
			RelatorioAjax.executarRelatorioUso(mostrarRel,numPag);
	}
	else
	{
		alert(msgErro);
...
function detalheFiltros(id){
	idLinha = id;
	window.open("relatorioUsoFiltros.jsp",'filtros','resizable=false,status=yes,menubar=no,scrollbars=yes,width=350,height=450');	
}
...
{		
	...
	RelatorioAjax.buscaRelatorioUsoPreenchido(mostrarRel,numPag,parametroOrdenacao,false);
}
...
function exportar(){
	RelatorioAjax.exportarUso(resExportar,1);
}
function resExportar(){
	$('exportar').src = '../../servlets/Exportar';
}
function imprimir(){
	window.open("/PortalOsx/templates/segAcesso/imprimir.jsp?tipo=2",'Impressão',		'resizable=yes,status=yes,toolbar=yes,menubar=no,scrollbars=yes,width=680,height=560');
}
</script>
</head>
