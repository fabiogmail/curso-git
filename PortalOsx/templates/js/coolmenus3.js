...
Visit www.dhtmlcentral.com/coolmenus/ for the latest version of the script.
...
function cm_checkLoaded(ev,ns){
	...
	if(document.layers){ 
		...
		if (coolFName.win.document.readyState == "complete" || coolFName.win.document.readyState == "interactive"){
			if(coolFName.win.document.body != null){
...
function cm_unloaded(ev){
	...
	if(document.layers && ev.target.name!=coolFName.frame){ 
		return; 
	}else{ 
		coolFName.nsload=0; 
		coolFName.loaded=0;
