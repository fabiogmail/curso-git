...
public class OpListaExportaDrill extends OperacaoAbs
{
	 private Vector m_Linhas = null;
	   ...
	   public boolean iniciaOperacao(String p_Mensagem) 
	   {
	      ...
	      try
	      {
	         ...
	         m_Args[2] = "src=\"/PortalOsx/templates/js/listadrillexportados.js\"";
	         m_Args[3] = "onLoad=\"PreloadImages('/PortalOsx/imagens/lixo0.gif','/PortalOsx/imagens/lixo1.gif'); IniciaDelecao()\"";
	         m_Args[4] = "exportadosdrilldown.gif";
	         m_Args[5] = m_Html.criaFormulario(NoUtil.getNo().getDiretorioDefs().getS_DIR_TMPL_FORMS(), "listadrillexportados.form", Args);
	   ...
	   public Vector montaLinhas() 
	   {
	      ...
	      if(m_Perfil==null){
	    	  if(DefsComum.s_CLIENTE.equalsIgnoreCase("Oi")){
	    		  ...
	    		  {
	    			  noTmp = (No) iter.next();
	    			  Vector tmp = noTmp.getConexaoServUtil().getListaRelExportadosXLSDrill(Integer.toString(Usuario.getIdPerfil()));
	    			  if(tmp != null)
	    	  ...
	    	  if(DefsComum.s_CLIENTE.equalsIgnoreCase("Oi")){
	    		  ...
	    		  {
	    			  noTmp = (No) iter.next();
	    			  Vector tmp = noTmp.getConexaoServUtil().getListaRelExportadosXLSDrill(m_Perfil);
	    			  if(tmp != null)
	    				  LinhasAux.addAll(tmp);
	      ...
	      if (/*LinhasAux != null ||*/ LinhasAux.size() > 0)
	      {
	         ...
	         for (int i = 0; i < LinhasAux.size(); i++)
	         {
	            ...
	            if (bPronto)
	            {
	            	if(m_Perfil != null)
	            	{
	            		 Coluna.add("<a href=\"javascript:AbreJanelaDrill('detalhar', '"+Usuario.getIdPerfil()+"','"+m_Perfil+"','"+ColunaAux.elementAt(1)+"')\" onmouseover=\"window.status='Clique para ver detalhes do relatório';return true;\" onmouseout=\"window.status='';return true;\"><img src=\"/PortalOsx/imagens/reldetalha.gif\" border=\"0\"></a>");
	            	}
	            	else
	            	{
	            		 Coluna.add("<a href=\"javascript:AbreJanelaDrill('detalhar', '"+Usuario.getIdPerfil()+"','"+Usuario.getPerfil()+"','"+ColunaAux.elementAt(1)+"')\" onmouseover=\"window.status='Clique para ver detalhes do relatório';return true;\" onmouseout=\"window.status='';return true;\"><img src=\"/PortalOsx/imagens/reldetalha.gif\" border=\"0\"></a>");
	            	}              
	            }
	            else
	            {
	            ...
	            if (bPronto)
	            {
	            	if(m_Perfil != null)
	            	{
	            		Coluna.add("<a href=\"/PortalOsx/templates/jsp/downloadExport.jsp?operacao=downloadExport&usuario="+m_Perfil+"&arquivo="+ColunaAux.elementAt(0)+"&diretorio="+NoUtil.getNo().getDiretorioDefs().getS_DIR_CDRVIEW()+"bdados/exportDrill/"+"\" onmouseover=\"window.status='Clique para fazer o download do relatório';return true;\" onmouseout=\"window.status='';return true;\"><img src=\"/PortalOsx/imagens/reldownload.gif\" border=\"0\"></a>");               	
	            	}
	            	else
	            	{	
	            		Coluna.add("<a href=\"/PortalOsx/templates/jsp/downloadExport.jsp?operacao=downloadExport&usuario="+Usuario.getIdPerfil()+"&arquivo="+ColunaAux.elementAt(0)+"&diretorio="+NoUtil.getNo().getDiretorioDefs().getS_DIR_CDRVIEW()+"bdados/exportDrill/"+"\" onmouseover=\"window.status='Clique para fazer o download do relatório';return true;\" onmouseout=\"window.status='';return true;\"><img src=\"/PortalOsx/imagens/reldownload.gif\" border=\"0\"></a>");
//	            		Coluna.add("<a href=\"/PortalOsx/templates/jsp/downloadExport.jsp\"><img src=\"/PortalOsx/imagens/reldownload.gif\" border=\"0\"></a>");
	   ...
	   public void montaTabela() 
	   {
	      ...
	      m_Html.m_Tabela.setLink("/PortalOsx/servlet/Portal.cPortal?operacao=listaExportaDrill&perfil="+Usuario.getIdPerfil());
