...
/**
 */
public class OpVerificaBilConversores extends OperacaoAbs 
{
    ...
    */
   public OpVerificaBilConversores() 
   {
   }
   ...
   public boolean iniciaOperacao(String p_Mensagem) 
   {
      try
      {
         setOperacao("Verificação de Configuração de Conversores");
         ...
         m_Args[2] = ""; // javascript
         m_Args[3] = ""; // onload
         m_Args[4] = "verificacfgconversores.gif";
         m_Args[5] = m_Html.criaFormulario(NoUtil.getNo().getDiretorioDefs().getS_DIR_TMPL_FORMS(), "verificabilhetadores.form", Args);
         m_Args[6] = m_Html.leFormulario(NoUtil.getNo().getDiretorioDefs().getS_DIR_TMPL_TXT(), "verificaconversores.txt", null);
         m_Html.enviaArquivo(m_Args);
         return true;
      }
      catch (Exception Exc)
      {
         System.out.println("OpVerificaBilConversores - iniciaOperacao(): "+Exc);
         Exc.printStackTrace();
         return false;
      }
   }
   ...
   public Vector montaLinhas() 
   {
      ...
      for (Iterator iter = NoUtil.listaDeNos.iterator(); iter.hasNext();)
      {
		  ...
		  {
      	  	 noTmp = (No) iter.next();
      	  	 Vector conversores =  noTmp.getConexaoServUtil().getListaCfgConversores();
      	  	 if(conversores != null)
      	  	 	ListaConfig.addAll(conversores);
		  }
   ...
   public void montaTabela()
   {
      ...
      m_Html.m_Tabela.setLink("/PortalOsx/servlet/Portal.cPortal?operacao=verificaBilConversores");
   ...
   public String verificaBilhetadores() 
   {
      ...
      for (Iterator iter = NoUtil.listaDeNos.iterator(); iter.hasNext();)
      {
		  ...
		  {
	          noTmp = (No) iter.next();
	          tmp = noTmp.getConexaoServUtil().verificaBilhetadores("conversor");
