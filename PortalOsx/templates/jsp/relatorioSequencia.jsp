...
function selecPagina(pagina){
	...
	OpRelSequenciaAjax.getRelatorioPagina(retRelatorio, pagina);	
}
	
function init(pagina){
	OpRelSequenciaAjax.setExportacao(1);//setar formato HTML como padrão para download.
	OpRelSequenciaAjax.getRelatorio(retRelatorio, pagina);	
...
function exportar(){
	OpRelSequenciaAjax.atualizaJasper(retExportar,"");
...
function detalheRel(valor, rel){
	if(rel == 0){
	
		OpRelSequenciaAjax.setBilhetador(valor);
		window.open('/PortalOsx/servlet/Portal.cPortal?operacao=apresentaRelatorioSequencia&subOperacao=relSequenciaDrillFaltante','drill','toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=875,height=580');
	}
	else{
		OpRelSequenciaAjax.setBilhetador(valor);
		window.open('/PortalOsx/servlet/Portal.cPortal?operacao=apresentaRelatorioSequencia&subOperacao=relSequenciaDrillRepetido','drill','toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=875,height=580');
	}
}

function finaliza(){
	OpRelSequenciaAjax.finaliza();
}
...
<table name="banner" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr> 
	...
	<table id="tabelaGeral" width="100%" height="100%" cellpadding="0" cellspacing="0" border="0">
		...
		<tr>
			<td height="20px">				
				<b>&ensp;Relat&oacute;rio de Sequ&ecirc;ncia</b>
			</td>
		</tr>
