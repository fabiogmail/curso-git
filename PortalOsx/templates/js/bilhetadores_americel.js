...
{
   if (NomeDoCampo == "Cod. area/Cod. Pre-Pago")
   {
      ...
      if (Ret != -1)
         return NomeDoCampo + " somente pode conter caracteres numéricos alem de \'-\'!";

      if (ValorDoCampo.charAt(2) != '-')
         return NomeDoCampo + " deve ser separado por \'-\', p.e., 00-000!";
   }
...
function Inclui()
{
   ...
   Retorno = VerificaCampo("Cod. area/Cod. Pre-Pago", document.form1.apelido.value, 6, 6);
   if (Retorno.length > 0)
   {
      alert (Retorno);
      return false;
...
function Exclui()
{
	...
	{
		alert ("Selecione um bilhetador para apagar!");
		return false;
	}
}
function Altera()
